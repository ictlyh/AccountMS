...
public class OutAccountDAO {
	 ...
	 * @return
	 */
	public TableOutAccount find(int id) {
		db = helper.getWritableDatabase();//初始化SQLiteDatabase对象
		Cursor cursor = db
				.rawQuery(
	 ...
	 * @return
	 */
	public List<TableOutAccount> getScrollData(int start, int count) {
		...
		while (cursor.moveToNext()) {
			//将遍历到的支出信息添加到集合中
			tb_outaccount.add(new TableOutAccount(cursor.getInt(cursor
